...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("ban")
    .setDescription("Ban the given user from the server")
    .addUserOption((op) =>
      op
        .setName("user")
  ...
  async execute(interaction, client) {
    ...
    if (
      ...
      return interaction.reply({
        content: `This user has higher roles than you, thus you cannot ban them`,
      });

    const emBan = new EmbedBuilder()
      .setColor(cs)
      .setTitle(`User has been banned [${t.tag}]`)
      .setDescription(
        `<@${memb.id}> has been banned from ${interaction.guild.name} by ${interaction.user.tag} ${success}.\n For the reason: \`${reason}\``
      )
      .setTimestamp();
    const em2 = new EmbedBuilder()
      ...
      .setDescription(
        `You have been banned from ${interaction.guild.name} by ${interaction.user.tag} ${error}.\n For the reason: \`${reason}\``
      )
      .setTimestamp();
    try {
